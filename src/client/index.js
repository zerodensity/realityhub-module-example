class ModuleExampleClient {
  async start() {
    ...
    this.api.on('statuschange', (e) => {
      ...
      autoButton.setProperty({ Value: started });
    ...
    autoButton.setProperty({ Value: statusResponse.status === 'started' });
    ...
    updateButton.addEventListener('click', () => {
      ...
      for (let i = 0; i < rows.length; ++i) {
        const row = rows[i];
        nodeData[`line${i + 1}`].value = row.querySelector('reality-property').getProperty().Value;
    ...
    this.containerElement.querySelectorAll('i.up-down-arrow').forEach((icon) => {
      icon.addEventListener('click', () => {
        if (autoButton.getProperty().Value) return;
    ...
    goBtn1.addEventListener('click', () => {
      this.realityWorlAPI.setNodeProperty({
        Value: location1.getProperty().Value,
    ...
    goBtn2.addEventListener('click', () => {
      this.realityWorlAPI.setNodeProperty({
        Value: location2.getProperty().Value,
        NodePath: 'UserTrack_0',
        PropertyPath: 'Input//UserTransform/0',
        InterpType: 'EaseInOut',
        Duration: 2,
  ...
  updateTableData(exchangeRates) {
    ...
    for (let i = 0; i < values.length; ++i) {
      ...
      realityProperty.setProperty({ Value: value });
